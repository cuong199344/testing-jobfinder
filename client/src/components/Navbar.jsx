  ...
  return (
    <div>
      <Menu as="div" className="inline-block text-left">
          ...
          >
            ...
            <div className="leading[80px] flex flex-col items-start">
              <p className="text-sm font-semibold ">
                {/* Nếu như user exit thì sẽ hiện firstName hoặc là company name */}
                {user?.firstName ?? user?.name}
              </p>
              <span className="text-sm text-blue-600 ">
                {/* Job tille hoặc là company email */}
        ...
        >
          <Menu.Items className="absolute z-50 right-2 
          mt-2 w-56 origin-top-right 
          divide-y dividfe-gray-100 rounded-md 
          bg-white shadow-lg focus:outline-none ">
            <div className="p-1 ">
              <Menu.Item>
                {/* Chuyển hướng đến profile user hoặc company một cách dynamic */}
                {({ active }) => (
...
const Navbar = () => {
  ...
  // Kiểm tra xem người dùng có mở bằng moble screen or not
  ...
  return (
    <>
      <div className="relative bg-[#f7fdfd] z-50 shadow-lg">
        <nav className="container mx-auto flex items-center justify-between p-5">
          ...
          <div className="hidden lg:block">
            {/* Nếu user log in thì phải show profile user */}
            {!user?.token ? (
              <Link to="/user-auth">
                <CustomButton
                  title="Sign In"
